...
import {
  ...
  FileSpreadsheet, X, Sun, Moon, UploadCloud, Cloud, CloudOff, Key, RefreshCw,
} from "lucide-react";
...
// Public raw URL for reading — no token, no rate limit. The raw CDN can cache
// for a few minutes; we cache-bust with a timestamp and the manual Refresh
// button forces a fresh pull, so viewers still get current data.
function ghRawUrl() {
...
export default function BatteryTrending() {
  ...
  const dirtyRef = useRef(false); // true once the user changes data locally (unpublished)

  // Pull the latest shared data from the repo. Skips overwriting if the user has
  // unpublished local changes, unless forced (manual refresh button).
  async function refreshData(force = false) {
    if (dirtyRef.current && !force) return; // don't clobber unpublished local edits
    setRefreshing(true);
  ...
  // On startup, load shared data. Falls back to SEED if missing/offline.
  useEffect(() => {
    refreshData(true);
    // Re-pull when the tab/app regains focus, so reopening shows current data
    // (unless there are unpublished local changes).
    function onVisible() {
  ...
  async function publishData() {
    ...
    if (result.ok) {
      ...
      setSyncStatus("live");
      dirtyRef.current = false; // published — local matches shared again
    ...
    <div className={"bt-root" + (dark ? " dark" : "")}>
      ...
      <header className="bt-top">
        ...
        <div className="bt-actions">
          ...
          <button
            className="bt-iconbtn-top"
            onClick={() => {
              if (dirtyRef.current && !window.confirm("You have unpublished changes. Refreshing will replace them with the latest published data. Continue?")) return;
              refreshData(true);
            }}
            disabled={refreshing}
            title={dirtyRef.current ? "Refresh — discards unpublished changes" : "Refresh to latest shared data"}
            aria-label="Refresh data"
          >
            <RefreshCw size={16} className={refreshing ? "bt-spin" : ""} />
          </button>
...
@keyframes bt-spin-kf{to{transform:rotate(360deg)}}
.bt-spin{animation:bt-spin-kf .8s linear infinite}
